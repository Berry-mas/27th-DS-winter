...
답변: Simple PG는 에피소드 전체 수익을 동일하게 사용하기 때문에 초반 구간에서 그래프가 크게 변동하며 120 기준선을 넘는 구간이 짧게 나타남.
    REINFORCE는 reward-to-go를 사용해 학습이 빠르게 안정화되며 150~350 구간을 꾸준히 유지했고, 후기에는 350 이상을 장기간 유지함.

질문 2: 왜 REINFORCE 방식이 Simple PG보다 성능이 더 안정적인지 설명하시오.
답변: REINFORCE는 각 시점별로 reward-to-go를 곱해 기여도가 큰 상태-행동 쌍에만 강한 업데이트를 주기 때문에 그라디언트 분산이 줄고, 
    에피소드 전역 리턴을 고정으로 쓰는 Simple PG보다 노이즈가 적음. 또한 최근 보상에 더 민감하게 반응해 정책이 빠르게 개선되어 안정적으로 높은 보상을 유지할 수 있음.
'''